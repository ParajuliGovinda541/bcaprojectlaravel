...
@include('layouts.message')
    <div class="grid grid-cols-3 px-44 gap-10 my-10">
        <div>
            <img src="{{asset('images/product/'.$product->photopath)}}" alt="" class="w-full h-96 object-cover rounded-lg">
        </div>
        <div class="border-l-2 px-2 col-span-2">
            <h2 class="text-3xl">{{$product->name}}</h2>

            @if ($product->oldprice !='')
            <p class="text-red-700 text-2xl  line-through font-bold">Rs. {{$product->oldprice}}/-</p>
            @endif
